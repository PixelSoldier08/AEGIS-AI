...
def speak_web(text):
    """Voice that works on Streamlit Cloud"""
...
if 'booted' not in st.session_state:
    ...
    with intro.container():
        ...
        with col2:
            ...
            bar = st.progress(0)
            status = st.empty()
            for i, s in enumerate(["LOADING CORES...", "SYNCING HUD...", "ONLINE."]):
                status.markdown(f"`{s}`")
                bar.progress((i + 1) * 33)
                time.sleep(0.8)
    ...
    if prompt := st.chat_input("Command AEGIS..."):
        ...
        with st.chat_message("assistant"):
            with st.spinner("Analyzing data streams..."):
                ...
                # speak_web(response) # Uncomment if you want him to talk every time
